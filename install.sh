...
install_composite_print_fonts() {
    ...
    log_info "Gmarket Sans…"
    curl -sfL --retry 3 --retry-delay 2 \
        "https://raw.githubusercontent.com/fonts-archive/GmarketSans/main/GmarketSansMedium.ttf" \
        -o "${font_root}/gmarketsans/GmarketSansMedium.ttf" || log_warn "Gmarket Sans Medium download failed"
    curl -sfL --retry 3 --retry-delay 2 \
        "https://raw.githubusercontent.com/fonts-archive/GmarketSans/main/GmarketSansBold.ttf" \
        -o "${font_root}/gmarketsans/GmarketSansBold.ttf" || true
    curl -sfL --retry 3 --retry-delay 2 \
        "https://raw.githubusercontent.com/fonts-archive/GmarketSans/main/GmarketSansLight.ttf" \
        -o "${font_root}/gmarketsans/GmarketSansLight.ttf" || true
...
setup_usb_print_native() {
    ...
    local _f _copied=0
    for _f in usbPrint.cjs connectWifi.cjs liveWifi.cjs wifiActivity.cjs package.json AGENT_VERSION mycutbox-ota-update.sh docker-compose.yml; do
