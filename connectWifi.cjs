...
const { createLiveWifiService } = require("./liveWifi.cjs");
const { createWifiActivity } = require("./wifiActivity.cjs");
const liveWifi = createLiveWifiService({ activity: createWifiActivity() });
...
async function runNmcli(args) {
  ...
  try {
    ...
    return {
      ok: false,
      stdout: err.stdout || "",
      stderr: err.stderr || "Command failed or timed out",
      cmd,
    };
  }
}
...
async function pollDir(absDir) {
  if (!watchers.has(absDir)) {
    ...
    liveWifi.watchDirectory(absDir);
  ...
  while (state.running) {
    try {
      await liveWifi.refreshSession(absDir);
      if (liveWifi.isActiveDirectory(absDir)) {
        await sleep(POLL_MS_FILES);
        continue;
      }
      // Legacy cleanup/discovery resumes after foreground live setup closes.
      if (!swept) { await sweepStaleProcessingFiles(absDir); swept = true; }
      if (liveWifi.isActiveDirectory(absDir)) continue;
      const files = await fsp.readdir(absDir);
      if (liveWifi.isActiveDirectory(absDir)) continue;
      const latest = pickLatestConnectWifiFile(files);
      if (latest && !liveWifi.isConnecting()) {
        const result = await liveWifi.runLegacyConnect(() =>
          liveWifi.isActiveDirectory(absDir)
            ? { busy: true }
            : processConnectJob(absDir, latest)
        );
    ...
    try {
      ...
      for (const m of afcMounts) {
        for (const rel of APP_REL_DIRS) {
          ...
          if (watchers.has(appDir)) {
            liveWifi.watchDirectory(appDir);
            continue;
          }
          if (!(await isDir(appDir))) continue;
          // A live watcher can recover independently of the legacy job watcher.
          liveWifi.watchDirectory(appDir);
          if (watchers.has(appDir)) continue;
          try {
...
(async () => {
  ...
  console.log(`[Init] Legacy: connect_wifi_YYMMDD_HHmm.json; live Wi-Fi: wifi_session.json (v2)`);
