...
const { createWifiActivity } = require("./wifiActivity.cjs");

const exec = promisify(execCb);
const wifiActivity = createWifiActivity();
...
/** printThis_* 유효기간: 오늘(Pi TZ) 생성 또는 생성 3시간 이내만 인쇄 */
const PRINT_MAX_AGE_MS = Math.max(
  0,
  Number(process.env.USB_PRINT_MAX_AGE_MS || 3 * 60 * 60 * 1000) || 3 * 60 * 60 * 1000
);
const CLOCK_PROBE_NAME = ".mycutbox_clock";
...
/**
 * iPad's "now" as seen through AFC. The Pi has no RTC: offline boots restore
 * the last saved time (possibly days old), while a file written over AFC is
 * stamped by the iPad, the same clock that stamped printThis_*. Null on failure.
 */
async function deviceNowMs(absDir) {
  const probe = path.join(absDir, CLOCK_PROBE_NAME);
  try {
    await fsp.writeFile(probe, "");
    return (await fsp.stat(probe)).mtimeMs;
  } catch {
    return null;
  }
}
...
function localDay(ms) {
  return new Date(ms).toLocaleDateString("sv-SE"); // YYYY-MM-DD in process TZ
}

/** Same local day, within PRINT_MAX_AGE_MS, or future-dated (skew) → print. */
function isFreshPrintJob(mtimeMs, nowMs) {
  return nowMs - mtimeMs <= PRINT_MAX_AGE_MS || localDay(mtimeMs) === localDay(nowMs);
}
...
async function pollDir(absDir) {
  ...
  while (state.running) {
    try {
      if (await wifiActivity.isActive(absDir)) {
        await sleep(POLL_MS_FILES);
        continue;
      }
      const files = await fsp.readdir(absDir);
      let nowMs; // resolved lazily: one AFC probe per scan that has a job

      for (const f of files) {
        // Only pick fresh jobs: ^printThis_
        if (!/^printThis_/i.test(f)) continue;
        // A job already handed to composeAndPrint finishes normally. Defer the
        // next job if Wi-Fi setup opened while that earlier job was printing.
        if (await wifiActivity.isActive(absDir)) break;
        ...
        let s1 = 0, s2 = 0, mtimeMs = 0;
        try { s1 = (await fsp.stat(abs)).size; } catch { continue; }
        await sleep(400);
        try { ({ size: s2, mtimeMs } = await fsp.stat(abs)); } catch { continue; }
        if (s1 !== s2 || s2 === 0) continue;

        // Stale jobs (e.g. queued while the Pi was off) must not print later.
        // Probe failure falls back to the Pi clock; files newer than a stale
        // Pi clock still count as fresh, so this fails open, never closed.
        if (nowMs === undefined) nowMs = (await deviceNowMs(absDir)) ?? Date.now();
        if (!isFreshPrintJob(mtimeMs, nowMs)) {
          try {
            await safeRename(abs, prefixedPath(abs, "expired_"));
            console.log(`[Expired] ${f} (created ${new Date(mtimeMs).toISOString()}, device now ${new Date(nowMs).toISOString()})`);
          } catch (e) {
            console.warn(`[Expired] rename failed: ${abs}`, e.message || e);
          }
          continue;
        }
    ...
    try {
      ...
      for (const m of afcMounts) {
        for (const rel of APP_REL_DIRS) {
          ...
          if (watchers.has(appDir) || !isDir(appDir)) continue;
