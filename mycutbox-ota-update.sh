...
OTA_UPDATE_SCRIPT_VERSION="1.2.5.5"
SELF_REEXEC_ARGS=("$@")
...
# Per-device hold for an explicitly installed, unreleased test runtime. Applies
# to timers, NetworkManager reconnects and the fleet watcher's direct entry.
# Remove the marker after that runtime has shipped to resume normal updates.
OTA_HOLD_FILE="${MYCUTBOX_OTA_HOLD_FILE:-${HOME}/.pi/ota-update.hold}"
if [ "$TRIGGER" != "network-down" ] && [ -f "$OTA_HOLD_FILE" ]; then
  printf '[OTA] Local runtime test hold is active: %s\n' "$OTA_HOLD_FILE"
  exit 0
fi
...
# 로컬에 pull된 mycutbox-rpi-agent 이미지가 버전마다 쌓여 디스크를 잡아먹는 것을 막는다.
AGENT_IMAGE_KEEP_COUNT="${AGENT_IMAGE_KEEP_COUNT:-3}"
...
# [Cleanup] mycutbox-rpi-agent 이미지가 버전 태그(pilot, v1.2.4.1, v1.2.4.2, ...)마다 로컬에
# 그대로 남아 디스크를 계속 잡아먹는다 — 태그가 다르면 digest가 같아도 별개 행으로 잡히므로,
# 먼저 digest 기준으로 중복 제거한 뒤 최신 AGENT_IMAGE_KEEP_COUNT개만 남기고 지운다.
# 지금 컨테이너가 실제로 물고 있는 이미지는 절대 후보에서 빼서, 막 올라온 버전을 실수로
# 지우는 사고를 막는다.
prune_old_agent_images() {
  local repo="${1:-ghcr.io/m1nzaii/mycutbox-rpi-agent}"
  local running_id
  running_id="$(docker inspect -f '{{.Image}}' mycutbox-composite-print 2>/dev/null || true)"
  local old_ids
  old_ids="$(docker images "$repo" --no-trunc --format '{{.ID}}	{{.CreatedAt}}' \
    | sort -u -k1,1 \
    | sort -k2 -r \
    | tail -n "+$((AGENT_IMAGE_KEEP_COUNT + 1))" \
    | awk -F'\t' '{print $1}')"
  [ -z "$old_ids" ] && return 0
  while IFS= read -r id; do
    [ -n "$id" ] || continue
    [ "$id" = "$running_id" ] && continue
    if docker rmi "$id" >/dev/null 2>&1; then
      log "[Cleanup] Pruned old agent image: $id"
    fi
  done <<< "$old_ids"
}
...
refresh_from_public_installer() {
  ...
  local f
  for f in mycutbox-ota-update.sh install.sh docker-compose.yml AGENT_VERSION usbPrint.cjs connectWifi.cjs liveWifi.cjs wifiActivity.cjs package.json; do
...
has_active_wifi_setup() {
  [ -f "${AGENT_DIR}/wifiActivity.cjs" ] || return 1
  node -e 'require(process.argv[1]).createWifiActivity({directory:require("node:path").join(require("node:path").dirname(process.argv[2]),"mycutbox-wifi")}).anyActive().then(active => process.exit(active ? 0 : 1), () => process.exit(1))' "${AGENT_DIR}/wifiActivity.cjs" "$GVFS_BASE"
}
...
network_try_reconnect() {
  ...
  # `nmcli device connect` 는 "아무 저장된 프로필"이나 올리기 때문에, 사용자가 앱에서
  # 고른 Wi-Fi 연결이 진행 중이면 그 연결을 끊고 예전 프로필로 되돌려 버린다.
  if has_active_wifi_setup; then
    log "Live Wi-Fi setup is active. Skipping network reconnect."
    return 0
  fi
...
handle_network_down() {
  ...
  # 연결을 바꾸는 중에는 잠깐 오프라인이 된다. 그 순간을 장애로 보지 않는다.
  if has_active_wifi_setup; then
    log "Live Wi-Fi setup is active. Ignoring network-down trigger."
    return 0
  fi
...
main() {
  ...
  # A successful Wi-Fi activation itself triggers network-online. Do not
  # replace/restart the process which still has to deliver its result to iPad.
  if has_active_wifi_setup; then
    log "Live USB Wi-Fi setup is active. Deferring OTA until a later trigger."
    exit 0
  fi
  ...
  if has_active_wifi_setup; then
    log "Live USB Wi-Fi setup started during update preparation. Deferring service restart."
    exit 0
  fi
